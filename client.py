...
import datetime, shad.encryption
...
class Bot:
	def __init__(self, app_name, phone_number=None, auth=None, displayWelcome=True, device=defaultDevice):
		if displayWelcome : print(f"shad library version {__version__}\n{__copyright__}\n☞ docs : http://shadlib.ml\n\nexecuting codes...\n")
		...
		self.enc = shad.encryption.encryption(self.auth)
	...
	def sendCode(phone_number):
		...
		enc = shad.encryption.encryption(tmp)
		while True:
			try:
	...
	def registerDevice(auth, device=defaultDevice):
		while True:
			try:
				enc = shad.encryption.encryption(auth)
...
class Socket:
	...
	def __init__(self, auth):
		self.auth = auth
		self.enc = shad.encryption.encryption(auth)
